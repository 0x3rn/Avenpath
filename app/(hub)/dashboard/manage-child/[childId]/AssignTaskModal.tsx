...
export default function AssignTaskModal({ childId }: { childId: string }) {
  ...
  const handleAssign = async () => {
    if (!query) {
      toast.error("Please enter a task or quiz name");
      return;
    }
  ...
  return (
    <>
      <button 
        ...
        className="flex items-center gap-2 bg-muted text-foreground border border-border font-bold px-5 py-2.5 rounded-xl hover:bg-muted/80 transition-colors shadow-sm"
      >
          ...
          <div className="bg-card border border-border w-full max-w-lg rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-xl font-extrabold flex items-center gap-2">
                <FileText className="w-5 h-5 text-muted-foreground" /> Assign Custom Task
              </h2>
              <button 
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-3">
                <label className="text-sm font-bold block">Task Type</label>
                <div className="flex gap-2">
                  ...
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl font-bold text-sm transition-colors border ${type === 'general' ? 'bg-foreground text-background border-foreground' : 'bg-muted border-border hover:bg-muted/80 text-muted-foreground'}`}
                  >
                    General Task
                  </button>
                </div>
              ...
              <div className="space-y-3">
                <label className="text-sm font-bold block">Task Name</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder={`e.g. ${type === 'quiz' ? 'Biology Midterm' : 'Read Chapter 4'}...`}
